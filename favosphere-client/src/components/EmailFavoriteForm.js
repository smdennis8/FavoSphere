...
function EmailFavoriteForm() {
    ...
    const { urlToScrape } = location.state;
    ...
    const getFavoriteSuggestions = async (url) => {
        const data = await callScraperFromUrl(url);
        setFavorite({
            favoriteId: 0,
    ...
    return <div className="container-fluid">
        <form onSubmit={handleSaveFavorite}>
            ...
            <div className="mb-3">
                ...
                <button className="btn btn-secondary" onClick={() => getFavoriteSuggestions(urlToScrape)}>
                    <i className="bi bi-trash"></i> Suggest
                </button>
                }
            </div>
...
export default EmailFavoriteForm;